...
void main(void) {
    ...
    /* Clear gpcfg to route GPO pins directly (not needed for EPWM, but good practice) */
    CT_CFG.gpcfg0_reg = 0;
    ...
    while (1) {
        ...
        if (freq != last_freq) {
            ...
            if (freq == 0) {
                /* Force output B low (on/off feature) */
                epwm->AQCSFRC = 0x0004;
            } else {
                 ...
                 * We want the smallest division factor to maximize resolution.
                 */
                ...
                if (best_tbprd > 0) {
                    ...
                    epwm->CMPB = (uint16_t)((tbprd_val + 1) / 2); /* 50% duty cycle */

                    /* Update clock division configuration */
                    uint16_t tbctl = (0x3 << 14) | (0x3 << 4) | (best_clkdiv << 10) | (best_hspclkdiv << 7) | 0;
                    epwm->TBCTL = tbctl;
                }
            }
        }
