 ...
 * Receives RPMsg messages from host and writes delay tuning value to Shared RAM.
 */
...
void main(void)
{
    ...
    // Initialize the Shared RAM value to 1000 cycles
    ...
    while (1) {
        ...
        if (__R31 & HOST_INT) {
            ...
            while (pru_rpmsg_receive(&transport, &src, &dst, payload, &len) == PRU_RPMSG_SUCCESS) {
                ...
                // Parse delay tuning value
                uint32_t val = parse_uint32((char *)payload, len);
                if (val > 0) {
                    *shared_frequency = val;
                }

                // Format response string without sprintf to reduce footprint and stack
                char response[64] = "Delay set to: ";
                uint16_t num_len = write_uint32_str(response + 14, val);
                strcpy(response + 14 + num_len, " cycles\n");
